	...
	IDetailLayoutBuilder& DetailBuilder)
{
	...
	UUOURewardCollectionMotionComponent* MotionComponent =
		CustomizedObjects.Num() == 1
			? Cast<UUOURewardCollectionMotionComponent>(CustomizedObjects[0].Get())
			: nullptr;
	...
	auto AddTimelineCategory = [
		...
		FeedbackComponent](
			...
			const FText& TimelineSearchText,
			EUOURewardMotionPhase MotionPhase)
	{
		...
		TimelineCategory
			.AddCustomRow(LOCTEXT(
				"TimelineMarkerGuideSearchText",
				"Presentation S O Intro Outro"))
			.WholeRowContent()
			[
				SNew(STextBlock)
				.Text(LOCTEXT(
		...
		TimelineCategory
			.AddCustomRow(TimelineSearchText)
			.WholeRowContent()
			[
				SNew(SBorder)
	...
	AddTimelineCategory(
		FName(TEXT("Reward|Motion|Appearance|Timeline")),
		LOCTEXT("AppearanceTimelineCategoryName", "Appearance Motion Timeline"),
		LOCTEXT("AppearanceTimelineSearchText", "Appearance Reward Timeline"),
		EUOURewardMotionPhase::Appearance);
	AddTimelineCategory(
		FName(TEXT("Reward|Motion|Collection|Timeline")),
		LOCTEXT("CollectionTimelineCategoryName", "Collection Motion Timeline"),
		LOCTEXT("CollectionTimelineSearchText", "Collection Reward Timeline"),
