...
// 물의 부피 계산에는 참여하지 않고, 지정한 Basin Target의 SurfaceWorldZ를 읽어 Owner Actor 또는 지정 컴포넌트의 높이만 갱신합니다.
...
public:
	...
	// 시작 시 Target/Platform 참조를 해석하고 초기 위치 차이를 저장합니다.
	virtual void BeginPlay() override;

	// 물이 장치에 의해 서서히 변할 수 있으므로 매 프레임 수면 높이를 따라갑니다.
	...
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Water Platform", meta = (ToolTip = "수면 높이에 맞춰 이동시킬 플랫폼 SceneComponent입니다. 비워두면 이름/태그 탐색 후 Platform Actor의 RootComponent를 사용합니다."))
	...
	// true이면 BeginPlay 시 현재 플랫폼 위치와 수면 높이의 차이를 자동으로 저장합니다.
	// 배치한 모습 그대로 물 수면을 따라 움직이게 만들 때 사용합니다.
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Water Platform", meta = (ToolTip = "게임 시작 시 현재 플랫폼 Z와 수면 Z의 차이를 자동으로 저장해 유지합니다."))
	bool bCaptureInitialOffsetOnBeginPlay = true;

	// bCaptureInitialOffsetOnBeginPlay가 꺼져 있을 때 사용할 수면 기준 Z 오프셋입니다.
	// 예를 들어 20이면 플랫폼 기준점이 수면보다 20cm 위에 위치합니다.
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Water Platform", meta = (ToolTip = "수면 높이에서 플랫폼을 얼마나 위/아래로 둘지 정하는 월드 Z 오프셋입니다."))
	float SurfaceOffsetZ = 0.0f;

	// 연결 그룹 전체가 같은 물 저장소로 동작할 때 그룹의 공통 수면 높이를 사용합니다.
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Water Platform", meta = (ToolTip = "켜져 있으면 Target 하나의 수면이 아니라 연결 그룹의 공통 수면 높이를 따라갑니다."))
	bool bUseConnectedGroupSurface = true;
	...
	// 현재 수면 높이를 기준으로 플랫폼 위치를 즉시 다시 계산합니다.
	UFUNCTION(BlueprintCallable, Category = "Water Platform")
	void RefreshPlatformPosition();

	// 현재 Target이 제공하는 수면 월드 Z를 반환합니다.
