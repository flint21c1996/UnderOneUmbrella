...
#include "UOUPuzzleConditionComponent.generated.h"

class UUOUWaterBasinPlatformComponent;
class UUOUWaterBasinTargetComponent;

// PuzzleConditionComponent가 어떤 값을 조건 판정 기준으로 사용할지 정합니다.
UENUM(BlueprintType)
enum class EUOUPuzzleConditionValueSource : uint8
{
...
// 조건 값을 기준값과 어떤 방식으로 비교할지 정합니다.
UENUM(BlueprintType)
enum class EUOUPuzzleConditionCompareMode : uint8
{
...
DECLARE_DYNAMIC_MULTICAST_DELEGATE_OneParam(FUOUPuzzleConditionEvent, const FUOUPuzzleConditionContext&, Context);

// WaterTile 또는 Platform의 값을 감시하다가 조건이 만족/해제될 때 Reaction 인터페이스 구현체에 알려주는 컴포넌트입니다.
// 이 컴포넌트는 조건 판정만 담당하고, 실제 퍼즐 반응은 IUOUPuzzleConditionActivatable을 구현한 컴포넌트가 담당합니다.
UCLASS(ClassGroup=(Puzzle), meta=(BlueprintSpawnableComponent, DisplayName="UOU Puzzle Condition"))
class UNDERONEUMBRELLA_API UUOUPuzzleConditionComponent : public UActorComponent
{
	GENERATED_BODY()

public:
	UUOUPuzzleConditionComponent();
	...
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Puzzle Condition", meta = (ToolTip = "여러 조건 컴포넌트를 구분하기 위한 이름입니다. ReactionComponent는 이 값을 보고 어떤 조건에서 호출됐는지 구분할 수 있습니다."))
	FName ConditionId = TEXT("WaterCondition");

	// 조건을 읽을 WaterTile Actor입니다. 비워두면 PlatformComponent의 TargetActor를 우선 사용합니다.
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Puzzle Condition|Target", meta = (ToolTip = "조건을 읽을 WaterTile Actor입니다. 해당 Actor에는 UOUWaterBasinTargetComponent가 있어야 합니다."))
	...
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Puzzle Condition|Target", meta = (ToolTip = "TargetWaterTileActor가 비어 있으면 같은 Actor의 WaterBasinPlatformComponent가 참조하는 WaterTile을 자동으로 사용합니다."))
	bool bAutoResolveWaterTileFromPlatform = true;

	// 조건 값으로 어떤 물/플랫폼 값을 사용할지 정합니다.
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Puzzle Condition|Rule", meta = (ToolTip = "조건 판정에 사용할 값을 정합니다. 예를 들어 수면 월드 Z, 채움 비율, 플랫폼 높이 등을 선택할 수 있습니다."))
	EUOUPuzzleConditionValueSource ValueSource = EUOUPuzzleConditionValueSource::WaterFillRatio;

	// 현재 값을 기준값과 어떻게 비교할지 정합니다.
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Puzzle Condition|Rule", meta = (ToolTip = "현재 값을 Threshold와 비교하는 방식입니다."))
	EUOUPuzzleConditionCompareMode CompareMode = EUOUPuzzleConditionCompareMode::GreaterOrEqual;

	// 조건 판정 기준 값입니다.
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Puzzle Condition|Rule", meta = (ToolTip = "조건 판정 기준 값입니다. 가득 참 조건은 ValueSource를 WaterFillRatio로 두고 Threshold를 1로 설정하면 됩니다."))
	float ThresholdValue = 1.0f;

	// BetweenInclusive 비교에서 사용하는 상한 값입니다.
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Puzzle Condition|Rule", meta = (ToolTip = "BetweenInclusive 비교에서 사용하는 상한 값입니다."))
	float UpperThresholdValue = 1.0f;

	// Equal, NotEqual 판정에서 허용할 오차입니다.
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Puzzle Condition|Rule", meta = (ClampMin = "0.0", ToolTip = "Equal, NotEqual 판정에서 허용할 오차입니다."))
	float Tolerance = 0.001f;

	// BeginPlay 시점에 조건을 한 번 평가합니다.
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Puzzle Condition|Trigger", meta = (ToolTip = "게임 시작 시 현재 물 상태로 조건을 한 번 평가합니다."))
	bool bEvaluateOnBeginPlay = true;

	// 매 프레임 조건을 다시 평가합니다. PlatformWorldZ처럼 물 이벤트 없이 값이 변하는 조건에 사용합니다.
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Puzzle Condition|Trigger", meta = (ToolTip = "매 프레임 조건을 다시 평가합니다. 보간 이동 중인 플랫폼 위치를 기준으로 조건을 잡을 때 사용합니다."))
	bool bEvaluateEveryTick = false;

	// true면 조건이 처음 만족되는 순간에만 Activate를 호출합니다.
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Puzzle Condition|Trigger", meta = (ToolTip = "켜져 있으면 조건이 처음 만족되는 순간에만 Activate를 호출합니다."))
	bool bTriggerOnce = false;

	// 같은 Actor에 붙은 컴포넌트 중 IUOUPuzzleConditionActivatable 구현체를 자동으로 호출합니다.
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Puzzle Condition|Reaction", meta = (ToolTip = "같은 Actor에 붙은 ReactionComponent들을 자동으로 찾아 호출합니다."))
	bool bNotifyOwnerComponents = true;

	// Owner Actor가 IUOUPuzzleConditionActivatable을 직접 구현했을 때 Actor에도 알림을 보냅니다.
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Puzzle Condition|Reaction", meta = (ToolTip = "Owner Actor가 인터페이스를 직접 구현한 경우 Actor에도 조건 알림을 보냅니다."))
	bool bNotifyOwnerActor = false;

	// 조건 변화 시 추가로 알림을 보낼 외부 Actor 목록입니다.
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Puzzle Condition|Reaction", meta = (ToolTip = "조건 변화 시 추가로 알림을 보낼 외부 Actor 목록입니다. Actor가 인터페이스를 구현했거나, Actor 내부 컴포넌트가 인터페이스를 구현하면 호출됩니다."))
	TArray<TObjectPtr<AActor>> ExplicitReactionActors;

	// ExplicitReactionActors 내부의 컴포넌트까지 인터페이스 구현 여부를 검사합니다.
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Puzzle Condition|Reaction", meta = (ToolTip = "ExplicitReactionActors 안에 붙은 컴포넌트까지 Reaction 인터페이스 구현 여부를 검사합니다."))
	bool bNotifyComponentsOnExplicitActors = true;

	// 조건 변화 시 추가로 알림을 보낼 외부 컴포넌트 목록입니다.
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Puzzle Condition|Reaction", meta = (ToolTip = "조건 변화 시 직접 호출할 외부 ReactionComponent 목록입니다."))
	TArray<TObjectPtr<UActorComponent>> ExplicitReactionComponents;

	// 조건 만족 상태가 바뀔 때마다 호출되는 블루프린트 이벤트입니다.
	UPROPERTY(BlueprintAssignable, Category = "Puzzle Condition|Event")
	FUOUPuzzleConditionEvent OnConditionChanged;

	// 조건이 false에서 true로 바뀌는 순간 호출되는 블루프린트 이벤트입니다.
	UPROPERTY(BlueprintAssignable, Category = "Puzzle Condition|Event")
	FUOUPuzzleConditionEvent OnConditionSatisfied;

	// 조건이 true에서 false로 바뀌는 순간 호출되는 블루프린트 이벤트입니다.
	UPROPERTY(BlueprintAssignable, Category = "Puzzle Condition|Event")
	FUOUPuzzleConditionEvent OnConditionUnsatisfied;
	...
	// 현재 조건 만족 여부를 반환합니다.
	UFUNCTION(BlueprintCallable, BlueprintPure, Category = "Puzzle Condition")
