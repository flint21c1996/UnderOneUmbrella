...
public:
	...
	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "Umbrella|Light", meta = (ToolTip = "켜면 빛 반사 상태에서 반사 허용 각도를 벗어난 입사광이 우산을 통과합니다. 기본값은 꺼짐이며, 뒤쪽 입사광은 반사되지 않고 우산에 차단됩니다."))
	bool bPassLightThroughOutsideReflectionAngle = false;

	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "Umbrella|Light", meta = (ClampMin = "0.0", ClampMax = "180.0", Units = "deg", ToolTip = "플레이어 전방(N)과 광원 쪽 방향(L)의 내적으로 판정하는 최대 반사 허용각입니다. 기본 100도에서는 측면과 뒤쪽 10도까지 반사하며, 이 범위를 벗어난 뒤쪽 빛은 반사되지 않고 우산에 차단됩니다."))
	float MaximumUmbrellaReflectionIncidenceAngle = 100.0f;
